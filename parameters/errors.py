...
def check_nan_metric(metric_dict, metric, metric_mean):
  ...
  for name, tensor in metric_dict.items():  # for name, age in dictionary.iteritems():  (for Python 2.x)
    if tensor == metric:
      metric_name = name
...
def check_data_frama_contain_features(data_frame: pd.DataFrame,
    features_names_list: list):
  """Raises a ValueError exception if data_frame don't contain any of wanted
  features"""
  df_headers = list(data_frame.columns.values)
  if not all(
      feature_name in data_frame for feature_name in features_names_list):
