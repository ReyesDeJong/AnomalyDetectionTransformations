"""
Transformer Ensemble OVA Simple network
"""
...
from models.transformer_ensemble_ova_od import EnsembleOVATransformODModel
...
class EnsembleOVATransformODSimpleModel(EnsembleOVATransformODModel):
  def __init__(self, data_loader: ZTFOutlierLoader,
      ...
      name='Ensemble_OVA_Transformer_OD_Simple_Model', **kwargs):
  ...
  def _get_model_list(self, input_shape, **kwargs):
    ...
    for transform_idx in range(self.transformer.n_transforms):
      network = DeepHits(input_shape=input_shape, n_classes=2, **kwargs)
