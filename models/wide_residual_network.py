...
def batch_norm():
  return BatchNormalization(axis=_get_channels_axis(), momentum=0.9,
                            epsilon=1e-5,
...
def conv2d(output_channels, kernel_size, strides=1):
  return Convolution2D(output_channels, kernel_size, strides=strides,
...
def dense(output_units):
  return Dense(output_units, kernel_initializer=_dense_kernel_initializer,
               kernel_regularizer=l2(WEIGHT_DECAY),
...
def _add_basic_block(x_in, out_channels, strides, dropout_rate=0.0):
  ...
  shortcut = x_in if is_channels_equal else conv2d(out_channels, 1, strides)(
      bn1)
...
def create_wide_residual_network(input_shape, num_classes, depth,
    widen_factor=1, dropout_rate=0.0,
  ...
  conv1 = conv2d(n_channels[0], 3)(
      inp)  # one conv at the beginning (spatial size: 32x32)
  conv2 = _add_conv_group(conv1, n_channels[1], n, 1,
                          dropout_rate)  # Stage 1 (spatial size: 32x32)
  conv3 = _add_conv_group(conv2, n_channels[2], n, 2,
                          dropout_rate)  # Stage 2 (spatial size: 16x16)
  conv4 = _add_conv_group(conv3, n_channels[3], n, 2,
                          dropout_rate)  # Stage 3 (spatial size: 8x8)

  out = batch_norm()(conv4)
  out = Activation('relu')(out)
  out = GlobalAveragePooling2D()(out)  # AveragePooling2D()(out)
  # out = Flatten()(out)
