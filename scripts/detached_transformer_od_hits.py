...
if __name__ == "__main__":
  ...
  outlier_arcsinh_score = np.arcsinh(norm_scores[~labels])
  thr = np.percentile(inlier_arcsinh_score, 96)

  arcsinh_scores_preds = arcsinh_scores <= thr
  accuracy = np.mean(arcsinh_scores_preds == labels)
  print('Accuracy: ', accuracy)

  bins = 100
  plt.hist(np.arcsinh(norm_scores[labels]), bins=bins, label='inliers',
           alpha=0.5)
  plt.hist(np.arcsinh(norm_scores[~labels]), bins=bins, label='outliers',
           alpha=0.5)
  thr = np.percentile(np.arcsinh(norm_scores[labels]), 96)
