...
def main():
  ...
  for loader_i in data_loaders:
    ...
    pickle_name = 'rank_incremental_%s_%s.pkl' % (
    ...
    for run_i in range(N_RUNS):
      ...
      for n_transforms_selected_so_far in range(len(trf_to_rank)):
        best_rank_found = 0
        best_rank_metric_in_this_run = 0
        for trf_i_idx in range(len(trf_to_rank)):
          ...
          if model_result_metric > best_rank_metric_so_far:
            ...
            print('best ', best_trf)
          if model_result_metric > best_rank_metric_in_this_run:
            best_rank_metric_in_this_run = model_result_metric
            best_trf_in_round = trf_to_perform
            print('best in this round', best_trf)
          results_run_i[trf_i_idx] = [trf_to_perform, results]
        trf_t0 = best_trf_in_round
        print(trf_t0)
