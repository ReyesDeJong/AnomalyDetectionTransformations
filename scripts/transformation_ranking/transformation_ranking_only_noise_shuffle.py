...
def main():
  ...
  for loader_i in data_loaders:
    ...
    print(y_test)
    pickle_name = 'n_s_only_rank_%s.pkl' % loader_i.name
    result_all_runs = {}
    for run_i in range(N_RUNS):
      indexes_for_power_set = list(range(len(power_set_clean))) + [-1, -2, -3,
                                                                   -4]
      indexes_for_power_set = list(range(len(power_set_clean)))[:1]  # + [-1]
      result_each_trf = {}
      for power_set_idx in range(1):
        if power_set_idx == -1:
        ...
        print(results['dirichlet']['acc_at_percentil'])
