"""
Here I run thins only with T0 + Noise + Shuffle old, and whats hapenning,
 iths that shuffle is made across al samples of batch
so, mean of shuffle para ouliers e inliers varies, given all,
 inlieras and outliers are together. lets Shuffle them, then results are the true bad ones
"""
...
def main():
  ...
  aux_transformer_correct = RankingTransformer(0, 0, 0, 0, 0, 0)
  aux_transformer_old = RankingTransformerOld(0, 0, 0, 0, 0, 0)
  trf_72 = Transformer()
  trf_9 = TransTransformer()
  n_tuple_array = list(range(aux_transformer_old.n_transforms))
  ...
  for loader_i in data_loaders:
    ...
    print(y_test)
    indxs_shuffle = np.arange(len(x_test))
    np.random.RandomState(42).shuffle(indxs_shuffle)
    x_test = x_test[indxs_shuffle]
    y_test = y_test[indxs_shuffle]
    print(y_test)
    pickle_name = '9_only_rank_%s.pkl' % loader_i.name
    result_all_runs = {}
    for run_i in range(N_RUNS):
      indexes_for_power_set = list(range(len(power_set_clean))) + [-1, -2, -3,
                                                                   -4]
      indexes_for_power_set = [-4]
      result_each_trf = {}
      for power_set_idx in indexes_for_power_set:
        if power_set_idx == -1:
          ...
          trf_to_perform = np.array(aux_transformer_old.transformation_tuples)[
            np.array(trforms_indx_set)]
          print(trf_to_perform)
          trfer = aux_transformer_old
          model = TransformODSimpleModel(
              loader_i, trfer, input_shape=x_train.shape[1:],
              results_folder_name=RESULT_PATH)
        model.fit(x_train, x_val, epochs=1000, patience=0)
        # results = model.evaluate_od(x_train, x_test[y_test != 1],
        #                             y_test[y_test != 1], loader_i.name,
        #                             'real', x_val)
        results = model.evaluate_od(x_train, x_test,
                                    y_test, loader_i.name,
                                    'real', x_val)
        print(results['dirichlet']['roc_auc'])
        # print(np.mean(results['dirichlet']['scores']))
        print(np.mean(results['dirichlet']['scores'][indxs_shuffle[:10]]))
        indxs_shuffle = np.arange(len(x_test))
        np.random.RandomState(42).shuffle(indxs_shuffle)
        print(results['dirichlet']['scores'][indxs_shuffle[:10]])
        x_test_2 = x_test[indxs_shuffle]
        y_test_2 = y_test[indxs_shuffle]
        results = model.evaluate_od(x_train, x_test[indxs_shuffle[:10]],
                                    y_test[indxs_shuffle[:10]], loader_i.name,
                                    'real', x_val)
        print(np.mean(results['dirichlet']['scores']))
        print(results['dirichlet']['scores'])
        # print(results['dirichlet']['scores'][indxs_shuffle[:10]])
