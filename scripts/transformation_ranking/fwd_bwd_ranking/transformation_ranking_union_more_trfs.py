...
def main():
    ...
    plus_kernel_noise_shift = transformer_for_ranking. \
        PlusKernelShuffleNoiseTransformer()
    ...
    for trf in transformer_list:
        # print(trf.n_transforms)
        # print(trf.transformation_tuples)
        union_ranker = UnionTransformRanker({
            param_keys.EPOCHS: EPOCHS_TO_USE,
            param_keys.TRAIN_N_TIMES: N_TRAIN_RUNS
        }, verbose_training=VERBOSE_TRAINING)
        union_ranker.rank_transformations(
            ztf_loader, hits_loader, TransformODSimpleModel, trf,
            verbose=VERBOSE)
        union_ranker.rank_transformations(
            hits_loader, ztf_loader, TransformODSimpleModel, trf,
            verbose=VERBOSE)
