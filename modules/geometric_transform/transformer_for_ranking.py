...
class RankingTransformation(object):
    def __init__(self, flip, tx, ty, k_90_rotate, gauss, log, mixed, noise,
        trivial=0):
        self.flip = flip
        self.tx = tx
        self.ty = ty
...
class RankingTransformer(AbstractTransformer):
    ...
    def _create_transformation_op_list(self):
        transformation_list = []
        for is_flip, tx, ty, k_rotate, is_gauss, is_log, is_mixed,\
            is_noise, is_trivial, in \
            self.transformation_tuples:
            transformation = RankingTransformation(
                is_flip, tx, ty, k_rotate, is_gauss, is_log, is_mixed, is_noise,
                is_trivial)
...
class PlusKernelShuffleNoiseTransformer(PlusKernelTransformer):
    def __init__(self, translation_x=8, translation_y=8, rotations=True,
        flips=True, gauss=True, log=True, mixed=1, noise=1,
    ...
    def _create_transformation_tuples_list(self):
        ...
        self.transformation_tuples += ((0, 0, 0, 0, 0, 0, self.mixed, 0),
                                       (0, 0, 0, 0, 0, 0, 0, self.noise))

    def _create_transformation_op_list(self):
        transformation_list = []
        for is_flip, tx, ty, k_rotate, is_gauss, is_log, is_mixed, is_noise, in \
            self.transformation_tuples:
            transformation = RankingTransformation(
