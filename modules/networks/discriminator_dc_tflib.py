...
from modules.networks.discriminator_tflib import DiscriminatorTflib
import tflib as lib
...
class DiscriminatorDCTflib(DiscriminatorTflib):
  ...
  def _build_network(self, inputs, params, training_flag):
    with tf.variable_scope(constants.DISCRIMINATOR, reuse=tf.AUTO_REUSE):
      ...
      output = lib.ops.linear.Linear('Discriminator.Output4', 4 * 3 * 3 * DIM,
                                     1, output)
