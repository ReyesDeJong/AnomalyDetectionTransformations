...
class FrameToInputWithFeatures(FrameToInput):
  ...
  def _init_df_attributes(self, df: pd.DataFrame):
    ...
    self.features_keys = self.features_names_list
    self.labels = []
    self.images = []
    self.features = []

  # def adding_idexes(self, dataframe: pd.DataFrame):
  #   if 'indexes' in self.features_names_list:
  #     dataframe['indexes'] = dataframe.index.astype(int)
  #   return dataframe
  #
  # def adding_data_source(self, dataframe: pd.DataFrame):
  #   if 'data_source' in self.features_names_list:
  ...
  #
  # def add_attributes_to_dataframe(self, df):
  ...
  def get_datadict(self) -> Dict[str, list]:
    ...
    if not isinstance(loaded_data, pd.DataFrame):
      ...
      df = loaded_data
      # df = self.add_attributes_to_dataframe(df)
      ...
      results = Parallel(n_jobs=self.n_cpus)(
          delayed(_subprocess_by_serie)(df.loc[i], self.class_dict,
                                        self.stamp_keys, self.features_keys) for
  ...
  def get_preprocessed_datasets_splitted(self) -> Dict[str, Dataset]:
    ...
    datasets_dict[general_keys.TEST] = \
      self.dataset_preprocessor.preprocess_dataset(test_dataset)
...
if __name__ == "__main__":
  ...
  params.update({
    ...
    param_keys.FEATURES_NAMES_LIST:
      ['non_detections', 'sgscore1', 'distpsnr1', 'sgscore2', 'distpsnr2',
       'sgscore3',
       'distpsnr3', 'isdiffpos', 'fwhm', 'magpsf', 'sigmapsf', 'ra', 'dec',
       'diffmaglim', 'rb', 'distnr', 'magnr', 'classtar', 'ndethist',
  ...
  print(np.mean((preprocessed_splits_1[split_name].meta_data[:, -1] ==
