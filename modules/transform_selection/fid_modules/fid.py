...
def get_activations(images, sess, batch_size=50, verbose=False):
  ...
  if batch_size > n_images:
    print(
        "warning: batch size %f is bigger than the data size. setting batch size to data size %f" % (
        batch_size, n_images))
    batch_size = n_images
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
  ...
  if not np.isfinite(covmean).all():
    msg = "fid_modules calculation produces singular product; adding %s to diagonal of cov estimates" % eps
    warnings.warn(msg)
