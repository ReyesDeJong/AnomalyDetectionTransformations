...
class DatasetDividerInt(DatasetDividerPercentage):
  ...
  def __init__(
      self, data_set_obj=None, test_size=100, validation_size=100,
      test_random_seed=42, val_random_seed=42):
    super().__init__(data_set_obj, test_size, validation_size, test_random_seed,
                     val_random_seed)
  ...
  def _train_test_split(self, data_array, data_label, test_size, random_state):
    ...
    result_dict = {'train_array': [], 'test_array': [], 'train_labels': [],
                   'test_labels': []}
    # print(label_values)
    ...
    return np.concatenate(result_dict['train_array']), \
