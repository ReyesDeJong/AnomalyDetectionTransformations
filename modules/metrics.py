...
def iou_and_dice_deprecated(labels, predictions, onehot_format=False):
  ...
  inter_area = tf.reduce_sum(
      tf.multiply(predictions_flatten, labels_flatten), axis=1)
  sum_area = tf.reduce_sum(
      predictions_flatten, axis=1) + tf.reduce_sum(labels_flatten, axis=1)
...
def iou_and_dice_v2(labels, predictions, is_onehot=True, average=True):
  ...
  inter_area_endo, sum_area_endo = inter_and_sum_areas(
      labels_endo, predictions_endo)
  ...
  inter_area_epi, sum_area_epi = inter_and_sum_areas(
      labels_epi, predictions_epi)
  ...
  inter_area_myo, sum_area_myo = inter_and_sum_areas(
      labels_myo, predictions_myo)
