    ...
    if batchnorm:  # batchnorm parameter is not None
      ...
      if activation == tf.nn.relu:
        outputs = batchnorm_layer(
            outputs, name="bn", scale=False,
            batchnorm=batchnorm, training=training)
      else:
        outputs = batchnorm_layer(
            outputs, name="bn", scale=True,
            batchnorm=batchnorm, training=training)

    outputs = activation(outputs)

    if drop_rate is not None:
      rate = drop_rate * tf.cast(training,
                                 tf.float32)  # if training is False rate=0
      # tf.summary.scalar('drop_rate', rate)
      outputs = tf.layers.dropout(outputs, rate=rate, name='dp')
...
def deconv2d(
  ...
  with tf.variable_scope(name):
    outputs = tf.layers.conv2d_transpose(
        ...
        kernel_initializer=kernel_initializer, data_format=data_format,
        bias_initializer=bias_initializer, name="deconv2d")
    # Why scale is false when using ReLU as the next activation
