...
from modules.data_loaders.base_line_loaders import save_roc_pr_curve_data, \
  get_class_name_from_index, load_hits4c
import datetime
...
def test_model_loading(transformer, loader, dataset_name='hits-4-c',
  ...
  transformed_data_path = os.path.join(
      save_dir,
      '%s_data_%s_%s_loading.pkl' % (transformer_name, dataset_name, tf_version))
  ...
  save_results_file(results_dir, dataset_name, single_class_ind, scores=scores,
                    labels=labels,
                    experiment_name='%s-loading-%s' % (transformer_name, tf_version))
  save_results_file(results_dir, dataset_name, single_class_ind,
                    scores=scores_simple,
                    labels=labels,
                    experiment_name='%s-simple-loading-%s' % (transformer_name,tf_version))
  save_results_file(results_dir, dataset_name, single_class_ind,
                    scores=scores_entropy,
                    labels=labels,
                    experiment_name='%s-entropy-loading-%s' % (transformer_name,tf_version))
  save_results_file(results_dir, dataset_name, single_class_ind,
                    scores=scores_xH,
                    labels=labels,
                    experiment_name='%s-xH-loading-%s' % (transformer_name, tf_version))
  mdl_weights_name = '{}_{}_{}_loading_{}_weights.h5'.format(transformer_name, dataset_name,
                                                          tf_version,
                                                          get_class_name_from_index(
                                                              single_class_ind,
                                                              dataset_name))
...
if __name__ == '__main__':
  ...
  # print(scores)
  start_time = time.time()
  scores = test_model_loading(transformer, load_hits4c,
                              dataset_name='hits-4-c', tf_version='tf1', transformer_name='transformed')
  print(
      "Time test_model_original(transformer, load_hits4c, dataset_name='hits-4-c', tf_version='tf1') %s" % utils.timer(
          start_time, time.time()),
      flush=True)
  print(scores)
